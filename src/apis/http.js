/*
 * @Author: renfeng 
 * @Date: 2020-09-04 17:46:46 
 * @Last Modified by: renfeng
 * @Last Modified time: 2020-09-17 11:33:07
 */
...
export function refreshToken() {
  ...
  headers.access_token = userToken.refreshToken;
  // headers.token = userToken.accessToken
  
  return new Promise((resolve) => {
    axios.request(apiPath + '/auth/token', {
      headers: headers,
      method: 'get',
      data: JSON.stringify({
...
export async function apiAxios(options) {
  ...
  const userToken = getToken();
  headers.access_token = userToken.accessToken;
